...
def clean_text_advanced(text: str, pages_content: List[Dict[str, Any]] = None) -> str:
    ...
    # Step 6: Remove excessive special symbols - keep only standard punctuation
    text = re.sub(r'[^\w\s\.\,\;\:\!\?\-\_\(\)\[\]\{\}\"\'\`\/\\\|\=\+\<\>\@\#\$\%\&\*\~\n\t]', ' ', text)
    
    # Step 7: Remove sequences of special characters (likely noise)
    ...
    # Step 9: Remove lines that are mostly noise
    lines = text.split('\n')
    cleaned_lines = []
    for line in lines:
        line_stripped = line.strip()
    ...
    # Step 10: Fix common OCR issues
    text = text.replace('II', 'I').replace('EE', 'E').replace('aa', 'a')
    
    # Step 11: Remove excessive repeated characters
