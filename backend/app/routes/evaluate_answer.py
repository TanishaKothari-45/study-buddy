...
async def preview_batch_answers(
    request: Request,
    file: UploadFile = File(...),
    use_standard_format: bool = Form(default=False),
    current_user: User = Depends(get_current_user)
):
    """
    Preview detected answers in PDF before processing.
    Returns detected answer boundaries without splitting.
    """
    try:
        # Save PDF temporarily
        job_id = str(uuid4())
        job_dir = settings.BASE_DIR / "data" / "temp" / f"preview_{job_id}"
        job_dir.mkdir(parents=True, exist_ok=True)
        ...
        if question_file:
            question_file_path = job_dir / f"question_{uuid4()}{Path(question_file.filename).suffix}"
        ...
        # Detect boundaries (without splitting)
        from ..utils.answer_splitter import detect_answer_boundaries
        import fitz
        ...
        preview_data = {
            ...
            "detected_answers": len(boundary_hits),
            "answers": []
        }
        
        # Calculate end pages for preview
        for idx, boundary in enumerate(boundary_hits):
            start_page = boundary["start_page"]
            q_num = boundary["q_num"]  # Actual question number
            ...
            # Use detected marks or estimate from page count
            estimated_marks = str(marks) if marks else ("10" if num_pages == 2 else "15" if num_pages == 3 else "?")
            
            preview_data["answers"].append({
                "answer_id": f"a{idx + 1}",
                "question_number": q_num,  # Actual question number
        ...
        return {
            ...
            "message": f"Detected {len(boundary_hits)} answers. Use /batch endpoint to process."
        }
    
    except Exception as e:
        logger.error(f"❌ Failed to preview batch: {e}", exc_info=True)
        raise HTTPException(500, str(e))
