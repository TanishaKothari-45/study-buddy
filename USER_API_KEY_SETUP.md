...
# 2. Set API Key
curl -X POST http://localhost:8000/api/v1/api-key/set \
  -H "Authorization: Bearer <your-token>" \
  -H "Content-Type: application/json" \
  -d '{"api_key": "AIzaSyYourActualGeminiKey"}'

# 3. Check Status
curl -X GET http://localhost:8000/api/v1/api-key/status \
  -H "Authorization: Bearer <your-token>"

# 4. Delete Key
curl -X DELETE http://localhost:8000/api/v1/api-key/delete \
  -H "Authorization: Bearer <your-token>"
```
